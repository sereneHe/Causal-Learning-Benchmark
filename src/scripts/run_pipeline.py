...
SUBPROCESS_TIMEOUT_METHODS = {"ExDBN", "RL", "CORL"}
...
def _run_method_in_subprocess(
    ...
    try:
        print(f"[STAGE] {context.method}: subprocess worker started.")
...
def _run_method_with_process_timeout(
    ...
    process.start()
    print(
        f"[STAGE] {context.method}: started subprocess pid={process.pid} "
    ...
    status, *data = payload
    if status == "ok":
        print(f"[STAGE] {context.method}: subprocess pid={process.pid} finished successfully.")
        ...
        try:
            for method in self.config.methods:
                ...
                if self.config.skip_existing and adj_path.exists():
                    print(f"[SKIP] {method}: existing result found at {adj_path}.")
                ...
                with self.mlflow.method_run(method):
                    print(
                        f"[START] {method}: dataset={self.config.dataset_name}, "
                        f"timeout={self.config.time_limit}s."
                    )
                    with Timer() as timer:
                        adj_matrix = self._run_with_timeout(context)

                    if adj_matrix is None:
                        print(f"[END] {method}: no result produced.")
                    ...
                    print(f"[END] {method}: completed in {timer.elapsed:.2f}s.")
