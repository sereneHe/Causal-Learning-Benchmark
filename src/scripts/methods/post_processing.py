...
def compute_metrics(adj_matrix: np.ndarray, true_matrix: np.ndarray) -> dict[str, float]:
    ...
    sid = int(_compute_sid(estimated, truth))
...
def _compute_sid(adj_matrix: np.ndarray, true_matrix: np.ndarray) -> int:
    est_reach = _reachability_matrix(adj_matrix)
    true_reach = _reachability_matrix(true_matrix)
    mask = ~np.eye(true_reach.shape[0], dtype=bool)
    return int(np.sum(est_reach[mask] != true_reach[mask]))


def _reachability_matrix(adj_matrix: np.ndarray) -> np.ndarray:
    graph = nx.DiGraph(np.asarray(adj_matrix, dtype=int))
    nodes = list(range(adj_matrix.shape[0]))
    graph.add_nodes_from(nodes)
    reachability = np.zeros((adj_matrix.shape[0], adj_matrix.shape[0]), dtype=bool)
    for source in nodes:
        descendants = nx.descendants(graph, source)
        if descendants:
            reachability[source, list(descendants)] = True
    return reachability
...
def merge_scores(
    ...
    elif "SID" in merged.columns:
        sid_df = merged[["Method", "SID"]].copy()
    else:
    ...
    final_df = category_df.merge(merged, on="Method", how="left")
    if "SID" not in final_df.columns and not sid_df.empty:
        final_df = final_df.merge(sid_df, on="Method", how="left")
