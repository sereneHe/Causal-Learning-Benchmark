...
class ICALiNGAM(_BaseLiNGAM, BaseLearner):
    ...
    def fit(self, X):
        ...
        # FastICA can yield extremely small or non-finite coefficients on some
        # real datasets. Use a finite fallback cost so Hungarian assignment can
        # still run instead of failing with "cost matrix is infeasible".
        abs_w_ica = np.abs(W_ica)
        eps = np.finfo(abs_w_ica.dtype).eps
        large_cost = 1.0 / eps
        cost_matrix = np.divide(
            1.0,
            abs_w_ica,
            out=np.full_like(abs_w_ica, large_cost),
            where=abs_w_ica > eps,
        )
        ...
        diagonal = np.diag(PW_ica)
        safe_diagonal = np.where(
            np.abs(diagonal) > eps,
            diagonal,
            np.where(diagonal >= 0, eps, -eps),
        )
        D = safe_diagonal[:, np.newaxis]
