...
class Reward(object):
    ...
    @staticmethod
    def _safe_median_distance(value):
        value = float(value)
    ...
    def cal_rewards(self, graphs, positions=None, ture_flag=False, gamma=0.98):
        ...
        for nu, (reward_, reward_list_) in enumerate(rewards_batches):
            ...
            normalised_reward = -reward_
            normal_batch_reward.append(normalised_reward)
            if normalised_reward > max_reward_batch:
                max_reward_batch = normalised_reward
    ...
    def cal_RSSi(self, i, graph_batch):
        ...
        if np.sum(col) < 0.1:
            ...
            if self.regression_type == 'LR':
                ...
                med_w = self._safe_median_distance(np.median(p_eu))
                p_eu_nor = p_eu / med_w
                K = np.exp(-0.5 * p_eu_nor)
                K = squareform(K)
                np.fill_diagonal(K, 1)
